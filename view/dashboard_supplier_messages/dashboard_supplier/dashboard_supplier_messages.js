...
class DashboardSupplierMessages {
  constructor(root = document) {
    ...
    listItems.forEach(item => {
      item.addEventListener('click', () => this.handleItemClick(item, listItems, threadContainers));
    });

    // Filtros (All / Unread / Approvals / System)
    filterButtons.forEach(btn => {
      btn.addEventListener('click', () => this.handleBtnClick(btn, filterButtons, listItems));
    });
    ...
    if (composeForm) {
      composeForm.addEventListener('submit', e => this.handleComposeFormSubmit(e, composeForm, messagesRoot));
    }
  }

  handleItemClick(item, listItems, threadContainers) {
  ...
  handleBtnClick(btn, filterButtons, listItems) {
  ...
  handleComposeFormSubmit(e, composeForm, messagesRoot) {
    e.preventDefault();
