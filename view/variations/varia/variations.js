...
class Variations {
  ...
  bindMenu() {
    ...
    this.menuList.addEventListener("click", (event) => {
      ...
      window.location.href =
        ...
        `?sku=${encodeURIComponent(skuProduct)}` +
  ...
  async getDefaultVariation() {
    ...
    try {
      const response = await fetch("../../controller/products/variations.php", {
        method: "POST",
      ...
      const skuVariation = String(data?.sku_default_variation || "").trim();
  ...
  getVariationDetails() {
    ...
    fetch("../../controller/products/variations.php", {
      ...
      .then((responseText) => {
        ...
        this.renderTypeVariationsSelect(
  ...
  renderTypeVariationsSelect(typeVariationsRaw, selectedTypeId) {
    ...
    const selected =
      selectedTypeId === null || selectedTypeId === undefined
        ? ""
        : String(selectedTypeId).trim();
  ...
  renderServerPreviews(current) {
    ...
    const downloadName = displayName.toLowerCase().endsWith(".pdf")
      ? displayName
      : `${displayName}.pdf`;
  ...
  async saveVariationDetails(goNext = true) {
    ...
    if (selectedParent) {
      skuParentVariation = String(
        selectedParent.dataset?.sku ||
    ...
    formData.append(
      "name_pdf_artwork",
      String(this.namePdfInput?.value || "").trim()
    );
  ...
  addNewVariation() {
    ...
    fetch("../../controller/products/variations.php", {
      ...
      .then((responseText) => {
        ...
        if (!json?.success) {
          alert(json?.message || "The variation could not be created.");
          return;
        }
