...
class PreviewLogic {
  ...
  bindVariationCollapse() {
    ...
    if (parent.dataset.collapseBound === "1") return;
  ...
  backBtn() {
    ...
    const skuv = current.searchParams.get(
      "sku_variation"
    );
  ...
  getDataProduct() {
    ...
    fetch(url, {
      ...
      .then((text) => {
        ...
        const company_name =
          json.find((x) => x.company_name)
            ?.company_name ?? "";

        const category_name =
          json.find((x) => x.category_name)
            ?.category_name ?? "";

        const group_name =
          json.find((x) => x.group_name)
            ?.group_name ?? "";

        const default_variation_id =
          json.find((x) => x.default_variation_id)
            ?.default_variation_id ?? "";

        const product_details =
          json.find((x) => x.product_details)
            ?.product_details ?? {};
  ...
  organizeCurrentVariation(
    ...
    try {
      ...
      /*
       * Always remove the previous content.
       *
       * This is necessary even when the new variation
       * contains no images, items or artwork. Otherwise,
       * the content from the previous variation remains.
       */
      ...
      /*
       * Rebuild thumbnails and show the updated media.
       */
      window.previewGallery?.refreshGallery?.(
        true
      );
  ...
  renderVariations(
    ...
    try {
      ...
      const blockHtml = `
        <div
          class="wrap-variations is-collapsible"
          aria-labelledby="${labelId}"
          data-type-id="${this.escapeHtml(typeId)}"
        >
          <button
            type="button"
            class="var-collapse-header"
            aria-expanded="false"
      ...
      if (firstDomId) {
        const selectVariationResult =
          this.SelectVariation(
            firstDomId
          );

        if (
          selectVariationResult === false
        ) {
          return false;
        }
      }
  ...
  renderItems(
    ...
    /*
     * Remove the previous wrapper before drawing
     * the items from the new selected variation.
     */
    this.deleteItems(typeId);
  ...
  renderImages(
    ...
    /*
     * Remove all previous images for this variation type.
     */
    this.deleteImages(typeId);
    ...
    /*
     * Refresh the gallery so that the main image,
     * thumbnails, arrows and autoplay use the new images.
     */
    window.previewGallery?.refreshGallery?.(
      true
    );
  ...
  renderArtwork(
    ...
    /*
     * Remove the artwork from the previous selected
     * variation before drawing the new one.
     */
    this.deleteArtwork(typeId);
    ...
    /*
     * Do not keep an empty artwork wrapper.
     */
    if (renderedArtwork > 0) {
      parent.appendChild(wrapper);
    }
  }
}
