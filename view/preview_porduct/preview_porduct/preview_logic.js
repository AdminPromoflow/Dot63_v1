...
class SupplierPreviewApp {
  ...
  renderReadiness(readiness) {
    ...
    if (this.elements.readinessSummary) {
      this.elements.readinessSummary.textContent = readiness.complete ? "Ready to submit for approval" : `${readiness.issues?.length || 0} item${readiness.issues?.length === 1 ? "" : "s"} need attention`;
  ...
  async submitForApproval() {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        const missing = Array.isArray(error.details?.missing) ? ` Missing: ${error.details.missing.join(", ")}.` : "";
