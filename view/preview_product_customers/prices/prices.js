...
export class PricesController {
  ...
  render(rows = []) {
    ...
    const sorted = [...rows].filter(row => String(row?.price_display_mode ?? "prices") === "prices").sort((a, b) => Number(a?.min_quantity) - Number(b?.min_quantity));
    ...
    for (const row of sorted) {
      ...
      range.textContent = Number.isFinite(max) && max >= min ? `${min.toLocaleString("en-GB")}–${max.toLocaleString("en-GB")} units` : `${min.toLocaleString("en-GB")}+ units`;
  ...
  async refreshVariationExtras() {
    ...
    const buttons = Array.from(document.querySelectorAll("#wrap-variations-group .var-option[data-variation-id]"));
    const ids = buttons.map(button => Number(button.dataset.variationId)).filter(id => Number.isFinite(id) && id > 0);
    if (ids.length === 0) {
      this.updateSummary();
      return;
    }
    ...
    try {
      ...
      const priceById = new Map((result.prices || []).map(row => [String(row.variation_id), Number(row.price)]));
      const pricedVariationIds = new Set((result.priced_variation_ids || []).map(id => String(id)));
  ...
  updateSummary() {
    ...
    this.setText(this.elements.unitHint, hasPrice ? hasUnavailableSelection ? "Choose an available option for this quantity" : `per unit at ${safeQuantity.toLocaleString("en-GB")} units` : "Pricing not configured");
  ...
  handlePriceClick(event) {
    const button = event.target.closest(".price-tier");
