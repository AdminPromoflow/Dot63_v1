class ItemsLogic {
  ...
  async getItemsDetails() {
    ...
    try {
      const response = await this.makeRequest(url, data);
      if (response?.success) this.items.drawItems(response.items);
    } catch (error) {
      console.error("Error:", error);
    }
  }
  ...
  renderMenuTop(items) {
    ...
    list.forEach(it => {
      ...
      li.insertAdjacentHTML('afterbegin', `<span class="lvl-dot" aria-hidden="true" style="
        ...
        "></span>`);
      const skuHidden = sku ? `<span class="sku-hidden" style="position:absolute; left:-9999px; width:1px; height:1px; overflow:hidden;">${escape(sku)}</span>` : '';
  ...
  handleMenuListClick(e, menuList, menuBtn) {
    ...
    window.location.href = `../../view/items/index.php?sku=${encodeURIComponent(sku_product)}&sku_variation=${encodeURIComponent(sku)}`;
  }
