...
class Variation {
  ...
  /**
   * Resolve the complete root-to-target path for a variation SKU belonging to
   * the current product. This is used when a generated combination is opened
   * from Products so every option in that combination can be preselected.
   */
  public function getVariationPathBySkuForProduct(): array
  {
      $productId = (int)($this->product_id ?? 0);
      $targetSku = trim((string)($this->sku_variation ?? ''));

      if ($productId <= 0 || $targetSku === '') {
          return ['success' => false, 'error' => 'product_id and sku_variation required'];
      }

      try {
          $pdo = $this->connection->getConnection();
          $stmt = $pdo->prepare("
              SELECT variation_id, parent_id, SKU
              FROM variations
          ...
          foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
              ...
              if (mb_strtolower(trim((string)($row['SKU'] ?? ''))) === mb_strtolower($targetSku)) {
                  $targetId = $variationId;
              }
          }

          if ($targetId <= 0) {
              return ['success' => false, 'error' => 'Variation does not belong to this product'];
          }
          ...
          while ($cursor > 0 && isset($nodes[$cursor])) {
              if (isset($visited[$cursor])) {
                  return ['success' => false, 'error' => 'Circular variation path detected'];
              }

              $visited[$cursor] = true;
              $path[] = $cursor;
              $cursor = (int)($nodes[$cursor]['parent_id'] ?? 0);
          }
          ...
          error_log('getVariationPathBySkuForProduct error: ' . $error->getMessage());
